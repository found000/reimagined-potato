...
public plugin_init() {
	...
	register_cvar("cm_connect_string", "");
	register_cvar("cm_disconnect_string", "Player %name from %country has joined the game!");
	saytext_msgid = get_user_msgid("SayText");
}

public plugin_precache() {
	register_cvar("cm_connect_sound", "buttons/button9.wav");
		...
		if (display_type & SHOW_CONNECT) {
			...
			for (new i=0; i < num; i++) {
				...
				if (display_type & PLAY_SOUND_CONNECT) {
					...
					if (connect_soundfile[stringlen - 1] == 'v' && connect_soundfile[stringlen - 2] == 'a' && connect_soundfile[stringlen - 3] == 'w') {
						client_cmd(player, "spk ^"sound/%s^"", connect_soundfile);
					}
					if (connect_soundfile[stringlen - 1] == '3' && connect_soundfile[stringlen - 2] == 'p' && connect_soundfile[stringlen - 3] == 'm') {
	...
	if (!is_user_bot(id)) {
		...
		if (display_type & SHOW_DISCONNECT) {
			...
			for (new i=0; i < num; i++) {
				...
				if (disconnect_soundfile[stringlen - 1] == 'v' && disconnect_soundfile[stringlen - 2] == 'a' && disconnect_soundfile[stringlen - 3] == 'w') {
					client_cmd(player, "spk ^"sound/%s^"", disconnect_soundfile);
				}
				if (disconnect_soundfile[stringlen - 1] == '3' && disconnect_soundfile[stringlen - 2] == 'p' && disconnect_soundfile[stringlen - 3] == 'm') {
